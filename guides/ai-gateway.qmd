...
minimal, extensible terminal coding agent that supports custom model providers
via a `models.json` configuration file.

#### 1. Install Node.js and Pi

Pi is distributed as an npm package. Install Node.js via `pixi global`, then
use it to install Pi:

```sh
# install Node.js (exposes node, npm, npx)
pixi global install nodejs

# install pi globally
npm install -g @mariozechner/pi-coding-agent

# add npm's install location to $PATH
...
Pi supports custom providers through `~/.pi/agent/models.json`. Create the
...
Paste the following into `nano` as the contents of `~/.pi/agent/models.json`.
...
The `apiKey` field is the **name of an environment variable** that holds your
key — it is not the key itself. Set that variable so Pi can read it:
...
```sh
echo 'export LITELLM_KEY="YOUR_API_KEY..."' >> ~/.bashrc
source ~/.bashrc
```

Save the `models.json` file (`Ctrl + O`, then `Ctrl + X` in nano).

::: {.callout-note}

**About `models.json`**

Each entry under `providers` defines a custom provider with:

- `baseUrl` — the gateway endpoint
- `api` — the API protocol Pi should use (`google-generative-ai`, `anthropic-messages`, or `openai-completions`)
- `apiKey` — the name of an environment variable containing your API key
- `models` — a list of model IDs to expose in Pi's model selector

The two providers above expose the DREAM Lab gateway's Gemini and Anthropic
models respectively. You can remove either block if you only have access to one
model family. The `models.json` file is reloaded each time you open Pi's model
selector (`/model`), so edits take effect without restarting.
...
To select a model, type `/model` (or press `Ctrl+L`) and choose from the
`dreamlab-gemini` or `dreamlab-anthropic` providers. Confirm the connection
with a simple question ("Are we connected?").
